...
import { postPatient } from '../services/api';
...
const PatientForm: React.FC<PatientFormProps> = ({ addPatient }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!documentPhoto) {
      ...
      hasError = true;
    } else if (documentPhoto && !documentPhoto.name.endsWith('.jpg')) {
    ...
    try {
      const newPatient = await postPatient(formData);
      ...
      setDocumentPhoto(null);
        ...
        if (error instanceof Error) {
            setGeneralError(error.message || 'Failed to register patient.');
        } else {
            setGeneralError(String(error) || 'Failed to register patient.');
        }
    }
  };
